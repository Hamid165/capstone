    ...
    <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4" required
                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-emerald-500 focus:border-emerald-500">{{ old('description', $product->description) }}</textarea>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <!-- Stok -->
            <div>
                <label for="stock" class="block text-sm font-medium text-gray-700 mb-1">Stok Saat Ini <span class="text-red-500">*</span></label>
        ...
        <!-- Foto Produk -->
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Foto Produk (Biarkan kosong jika tidak diganti)</label>
            <div class="flex items-center gap-4">
                <div class="w-24 h-24 bg-gray-100 rounded-lg border-2 border-dashed border-gray-300 flex items-center justify-center overflow-hidden relative">
                    <img id="preview-image" src="{{ $product->image ? asset('storage/' . $product->image) : '#' }}" 
                         alt="Preview" class="w-full h-full object-cover {{ $product->image ? '' : 'hidden' }}">
                    <i id="placeholder-icon" class="fas fa-image text-gray-400 text-2xl {{ $product->image ? 'hidden' : '' }}"></i>
                </div>
                <div class="flex-1">
        ...
        <div class="flex items-center justify-between bg-emerald-50 p-4 rounded-lg border border-emerald-100">
            <div>
                ...
                <p class="text-xs text-emerald-700">Aktifkan untuk menampilkan badge "Best Seller".</p>
            </div>
            <label class="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" name="is_best_seller" class="sr-only peer" {{ old('is_best_seller', $product->is_best_seller) ? 'checked' : '' }}>
